...
if (length(errorIndexes) > 0)
{
    ...
    margin <- wx * yi
    ex <- lossLog(margin)
    w <- w + eta * xi * yi * sigmoidFunction(-wx * yi) ##расчитываем градиентный шаг для итерации
...
test <- function(ObjectsCountofEachClass) {
  ...
  colors <- c("green2", "red")
  plot(xlNorm[, 1], xlNorm[, 2], pch = 21, asp = 1, main="Линейные алгоритмы классификации. Логистическая регрессия.")
