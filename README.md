...
```  
Для ковариационной восстановления матрицы 𝛴:  
```diff  
...
Разделяющая поверхность между классами s и t - это геометрическое место точек x ∈ X таких, что максимум в формуле байесовского классификатора, представленной выше, достигается одновременно при y = s и y = t:  
...
После того как мы прологарифмируем ее и подставим в полученную после этого формулу с разностью логарифмов, то получим формулу коэффициентов разделяющей линии:  
...
В программной реализации нахождение коэффициентов подстановочного алгоритма (при уравнении кривой: a*x1^2 + b*x1*x2 + c*x2 + d*x1 + e*x2 + f = 0) выглядит так:  
...
2. Здесь также по 250 элементов в каждом классе. Видим, что эллипсоиды параллельны осям координат:  
...
```   
![plugin2](https://user-images.githubusercontent.com/43229815/50232948-5e78ec80-03c3-11e9-9cc8-353e23729f9f.png)  
3. Здесь по 50 эелементов каждом классе. Ковариационные матрицы не равны, разделяющая плотность является квадратичной и прогибается таким образом, что менее плотный класс охватывает более плотный.  
Ковариационные матрицы:  
```diff  
Sigma1 <- matrix(c(3, 0, 0, 1), 2, 2)
Sigma2 <- matrix(c(10, 0, 0, 15), 2, 2)
``` 
![plugin3](https://user-images.githubusercontent.com/43229815/50233126-cb8c8200-03c3-11e9-8c98-2206397f48a2.png)
...
![](https://camo.githubusercontent.com/65a258474daf14d2a0a47e3ae10e252e78cae132/68747470733a2f2f6c617465782e636f64)  
